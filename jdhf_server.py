...
from jdhf import order, query_order, query_order_im, real_url
...
def createOrderAppstore():
    ...
    print(param)
    threading.Thread(target=order, args=(ck, order_me, amount, phone)).start()
    return 'success'
...
@app.route('/queryOrderImmediate', methods=['POST'])
